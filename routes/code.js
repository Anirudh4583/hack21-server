...
router.post('/donate/:ipfs', (req,res)=>{
    try{
        ...
        const code = await Code.findOne({ ipfsDest: ipfs });
        var user = await User.findOne({ public_address: code.ownerAddress });

        var result = await contract.methods.crowdFunding(ipfs).send({
...
router.post('/buy/:ipfs', (req,res)=>{
    try{
        ...
        var result = await contract.methods.trade(ipfs, code.ownerAddress).send({
            from: address,
            gas: 200000,
            gasPrice: 200000000,
            value: value
